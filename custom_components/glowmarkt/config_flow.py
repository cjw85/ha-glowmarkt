"""Config flow for the Glowmarkt integration."""
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        glowmarkt = await hass.async_add_executor_job(
            GlowClient, data["username"], data["password"]
        )
        ...
        _LOGGER.error("Glow authentication response was invalid: %s", ex)
        raise ValueError("Authentication failed") from ex

    _LOGGER.debug("Successful Post to %sauth", glowmarkt.url)
    return {"title": "Glowmarkt"}
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        try:
            ...
            _LOGGER.debug("HTTP error during config flow authentication: %s", ex)
            if ex.response is not None and ex.response.status_code in (401, 403):
    ...
    async def async_migrate_entry(
        ...
        """Keep legacy entries on the current config-entry version."""
        _LOGGER.debug("Migrating from version %s", config_entry.version)
        return True


class OptionsFlowHandler(config_entries.OptionsFlow):
    """Handle an options flow for Glowmarkt."""
